...
class ViT(nn.Module):
    def __init__(self, img_size=224, patch_size=16, stride_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
                 ...
                 drop_path_rate=0., norm_layer = nn.LayerNorm,cfg = None):
        ...
        self.num_token = num_patches//2

        self.pos_drop = nn.Dropout(p=drop_rate)
        self.pos_drop1 = nn.Dropout(p=0)
        ...
        self.cfg = cfg
        print('self.cfg.BLOCK_NUM:'+str(self.cfg.BLOCK_NUM))
    ...
    def forward_features(self, x):
        ...
        if self.training:
            ...
            num_patches = self.num_token

        index1 = torch.LongTensor(random.sample(range(105), num_patches))  # 105
        index1, _ = torch.sort(index1)
        index1 = index1.cuda().detach()

        index2 = torch.LongTensor(random.sample(range(105, 210), num_patches))  # 105 210
        ...
        x_1 = torch.index_select(x, 1, index1)  # [64, 90, 768]
        x_2 = torch.index_select(x, 1, index2)  # [64, 90, 768]

        cls_token = self.cls_token.expand(B, -1, -1)

        x = torch.cat((cls_token, x), dim=1) + self.pos_embed
        x_1 = torch.cat((cls_token, x_1), dim=1) + pos_embed1
        x_2 = torch.cat((cls_token, x_2), dim=1) + pos_embed2

        x = self.pos_drop(x)

        for blk in self.blocks[:self.cfg.BLOCK_NUM]:
            x = blk(x)
            x_1 = blk(x_1)
            x_2 = blk(x_2)
        ...
        return x10, x10_1, x10_2, x9, x9_1, x9_2
