...
class CenterTripletLoss1(nn.Module):
   ...
   [(arxiv)](https://arxiv.org/abs/2008.06223).
    ...
    def __init__(self, margin=0.3):
        super(CenterTripletLoss1, self).__init__()
        self.margin = margin
    ...
    def forward(self, feats, labels):
        ...
        for i in range(n):
            # if(i<8):
            #     rgb_an.append( dist[i][mask[i]==0].max().cpu().detach().numpy())
            #     # print('rgb:'+str(d))
            # else:
            #     ir_an.append( dist[i][mask[i]==0].max().cpu().detach().numpy())

            dist_ap.append(dist[i][mask[i]].max().unsqueeze(0))
            dist_an.append(dist[i][mask[i] == 0].min().unsqueeze(0))
        # print('rgb {}'.format(np.mean(rgb_an, axis=0)))
        # print('ir {}'.format(np.mean(ir_an, axis=0)))
